...
import { fetchExpensePaymentForReceipt } from '../api/expensePaymentQueries'
...
export default function ExpenseReceiptView({ paymentId, onClose }) {
  ...
  useEffect(() => {
    ...
    setError(null)
    Promise.all([
      fetchExpensePaymentForReceipt(paymentId),
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Georgia', 'Times New Roman', serif; font-size: 13px; color: #111; background: #fff; line-height: 1.5; max-width: 420px; margin: 0 auto; }
...
<body>
  ...
  <div class="section">
    ...
    <div class="row"><span class="row-label">Description</span><span class="row-value">${e?.description || '—'}</span></div>
    <div class="row"><span class="row-label">Payee</span><span class="row-value">${e?.payee_name || '—'}</span></div>
  </div>
  <hr />
  <div class="section">
    <div class="section-title">Payment Details</div>
    <div class="row"><span class="row-label">Amount</span><span class="row-value amount-value">${currency} ${amt}</span></div>
    <div class="row" style="justify-content:flex-end"><span class="direction">Paid for expense</span></div>
  ...
  const handlePrint = () => {
    ...
    setTimeout(() => {
      try { iframe.contentWindow.print() } catch (e) { showToast('Could not print. Try allowing popups for this site.', 'error') }
      setTimeout(() => { if (iframe.parentNode) iframe.parentNode.removeChild(iframe) }, 1000)
    }, 300)
  }

  const e = data?.payment?.expenses || {}
          ...
          <>
            ...
            <div className="r-body">
              ...
              <div className="r-section">
                <div className="r-section-title">Expense Details</div>
                <div className="r-row"><span className="r-row-label">Description</span><span className="r-row-value">{e?.description || '—'}</span></div>
                <div className="r-row"><span className="r-row-label">Payee</span><span className="r-row-value">{e?.payee_name || '—'}</span></div>
              </div>
              <hr className="r-divider" />
              <div className="r-section">
                <div className="r-section-title">Payment Details</div>
                <div className="r-row"><span className="r-row-label">Amount</span><span className="r-row-value r-amount-value">{currency} {Number(data?.payment?.amount || 0).toLocaleString('en-PK', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span></div>
                <div className="r-row" style={{ justifyContent: 'flex-end' }}><span className="r-direction">Paid for expense</span></div>
