-- ============================================================
-- 0023 — Multi-user roles: tables, policies, functions
-- (Enum values 'admin' and 'tailor' were added in 0022.)
-- ============================================================

-- 1. Migrate existing 'staff' rows to 'admin'
...
-- 3. Add invited flag to tailors
ALTER TABLE tailors ADD COLUMN IF NOT EXISTS invited BOOLEAN NOT NULL DEFAULT false;

-- 4. RLS: work_assignments — tailors can only see their own rows
...
-- 5. RLS: profiles — tailors can only see their own profile row
...
-- 6. Function: get my tailor_id for the current logged-in tailor
...
-- 7. Function: tailor marks a work assignment stage complete
...
-- 8. Function: safe order view for tailors (no price/balance columns)
...
-- 9. Function: tailor work history
...
-- 10. Function: tailor's own earnings (no parameters — resolves from auth)
...
-- 11. Function: list team members for owner/admin
...
-- 12. Function: remove team member (owner only)
